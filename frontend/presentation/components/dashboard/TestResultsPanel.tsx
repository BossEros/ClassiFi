...
const STATUS_CONFIG: Record<string, { icon: typeof CheckCircle; color: string; bgColor: string }> = {
    'Accepted': { icon: CheckCircle, color: 'text-green-400', bgColor: 'bg-green-500/20' },
    'Wrong Answer': { icon: XCircle, color: 'text-red-400', bgColor: 'bg-red-500/20' },
    'Time Limit Exceeded': { icon: Clock, color: 'text-yellow-400', bgColor: 'bg-yellow-500/20' },
    'Memory Limit Exceeded': { icon: AlertTriangle, color: 'text-orange-400', bgColor: 'bg-orange-500/20' },
    'Compilation Error': { icon: XCircle, color: 'text-red-400', bgColor: 'bg-red-500/20' },
    'Runtime Error': { icon: AlertTriangle, color: 'text-red-400', bgColor: 'bg-red-500/20' },
    'Processing': { icon: RefreshCw, color: 'text-blue-400', bgColor: 'bg-blue-500/20' },
    'Internal Error': { icon: AlertTriangle, color: 'text-gray-400', bgColor: 'bg-gray-500/20' },
};
...
export function TestResultsPanel({ submissionId }: TestResultsPanelProps) {
    ...
    const fetchResults = useCallback(async () => {
        try {
            setIsLoading(true);
            setError(null);
            const response = await getTestResults(submissionId);
            if (response.data?.data) {
                setResults(response.data.data);
            } else if (response.error) {
                // No results yet is fine
                setResults(null);
            }
        } catch (err) {
            ...
            <CardContent>
                ...
                {!results ? (
                    <div className="text-center py-8">
                        ...
                        <p className="text-sm text-gray-500">Tests were not run before this submission</p>
                    </div>
                ) : (
...
function TestResultItem({ result, index, isExpanded, onToggle }: TestResultItemProps) {
    ...
    return (
        <div className="rounded-lg border border-white/10 overflow-hidden">
            ...
            >
                ...
                <div className="flex items-center gap-3">
                    <span className={`text-sm px-2 py-0.5 rounded ${config.bgColor} ${config.color}`}>
                        {result.status}
                    </span>
                    {!result.isHidden && (
                ...
                <div className="p-4 border-t border-white/10 space-y-3 bg-black/20">
                    ...
                    {result.actualOutput && (
                        <div>
                            ...
                            <pre className={`p-2 rounded text-xs overflow-x-auto font-mono ${isAccepted ? 'bg-green-500/10 text-green-300' : 'bg-red-500/10 text-red-300'}`}>
                                {result.actualOutput}
                            </pre>
                        </div>
                    )}
