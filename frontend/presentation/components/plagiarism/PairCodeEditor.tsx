...
export const PairCodeEditor: React.FC<PairCodeEditorProps> = ({
    ...
    const sortedFragments = [...fragments].sort((a, b) => {
        ...
        return (aRegion.startRow - bRegion.startRow) * 10000 +
            (aRegion.startCol - bRegion.startCol);
    });

    // Get the region for this side from a fragment
    const getRegion = useCallback((fragment: MatchFragment): CodeRegion => {
    ...
    const getFragmentAtPosition = useCallback((lineNumber: number, column: number): MatchFragment | null => {
        ...
        for (const fragment of sortedFragments) {
            ...
            const inRowRange = region.startRow + 1 <= lineNumber && lineNumber <= region.endRow + 1;
            ...
            const length = (region.endRow - region.startRow + 1) * 10000 +
    ...
    const buildSelections = useCallback(() => {
        ...
        for (const fragment of sortedFragments) {
            ...
            if (region.startRow === region.endRow || region.startRow === region.endRow - 1) {
                selections.push({
                    fragment,
                    range: {
    ...
    const areFragmentsEqual = (a: MatchFragment | null, b: MatchFragment | null): boolean => {
        if (!a || !b) return false;
        return a.leftSelection.startCol === b.leftSelection.startCol &&
    ...
    const scrollToFragment = useCallback((fragment: MatchFragment) => {
        if (!editorRef.current) return;
    ...
    useEffect(() => {
        ...
        editorRef.current.onDidChangeCursorPosition((e) => {
            ...
            const fragment = getFragmentAtPosition(e.position.lineNumber, e.position.column);
        ...
        editorRef.current.addAction({
            ...
            run: () => {
                ...
                const currentIndex = selectedFragment
                    ? sortedFragments.findIndex(f => areFragmentsEqual(f, selectedFragment))
                    : -1;
                const nextIndex = currentIndex === -1 || currentIndex === sortedFragments.length - 1
                    ? 0
                    : currentIndex + 1;
