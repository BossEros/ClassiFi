...
export function DatePicker({
  ...
  // Calendar view state
  const [viewDate, setViewDate] = React.useState(dateObj || new Date());
  ...
  const handleDateSelect = (day: number) => {
    ...
    // Pass strictly just the date part as an ISO-like string "YYYY-MM-DD"
    // The consumer (BasicInfoForm) splits by 'T' anyway or expects ISO
    // But to maintain simple ISO compatibility as expected by the prop type:
    // We create a "UTC" date that matches the local Y-M-D
    ...
    <div className="w-full space-y-2">
      ...
      <Popover
        ...
        trigger={
          <div
            className={cn(
              "flex items-center justify-between w-full h-11 px-4 rounded-xl border transition-all duration-200 group",
              disabled
