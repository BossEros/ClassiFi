...
import type { AdminUser, EnrolledStudent } from '@/business/services/adminService'
...
export function AdminAddStudentModal({ isOpen, onClose, onSuccess, classId, existingStudents }: AdminAddStudentModalProps) {
    ...
    const handleAddStudent = async (student: AdminUser) => {
        try {
            ...
            showToast(`Successfully enrolled ${student.firstName} ${student.lastName}`, 'success')
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div
                className="w-full max-w-lg bg-[#0B1120] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200"
                            ...
                            {students.map((student) => (
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-500/10 text-blue-400 hover:bg-blue-500 text-xs font-medium hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed group-hover:shadow-[0_0_10px_rgba(59,130,246,0.2)]"
                                    >
