...
import { X, BookOpen, Calendar, Clock, Loader2, Plus, ChevronDown } from 'lucide-react'
...
export function AdminCreateClassModal({ isOpen, onClose, onSuccess, teachers, classToEdit }: AdminCreateClassModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.academicYear) {
            ...
            setError('Academic year format must be YYYY-YYYY (e.g., 2024-2025)')
            return
        } else {
            const [startYear, endYear] = formData.academicYear.split('-').map(Number)
            if (endYear !== startYear + 1) {
                setError('End year must be exactly 1 year after start year')
                return
            }
        }
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : `Failed to ${classToEdit ? 'update' : 'create'} class`)
        } finally {
            setIsLoading(false)
        }
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl transform overflow-hidden rounded-2xl bg-slate-900/95 p-6 text-left shadow-2xl transition-all border border-white/10 backdrop-blur-xl max-h-[90vh] overflow-y-auto custom-scrollbar">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h3 className="text-xl font-bold text-white tracking-tight">{classToEdit ? 'Edit Class' : 'Create Class'}</h3>
                        <p className="text-sm text-gray-400 mt-1">{classToEdit ? 'Update existing class details' : 'Add a new class to the academic roster'}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-gray-400 ml-1">Class Name</label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-gray-400 ml-1">Teacher</label>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-gray-400 ml-1">Year Level</label>
                                <div className="relative">
                                    <select
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-gray-400 ml-1">Semester</label>
                                <div className="relative">
                                    <select
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-gray-400 ml-1">Academic Year</label>
                                <input
                                    type="text"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-medium text-gray-400 ml-1">Class Days</label>
                            <div className="flex flex-wrap gap-2">
                                {DAYS.map(day => (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${formData.scheduleDays.includes(day)
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-gray-400 ml-1">Start Time</label>
                                <input
                                    type="time"
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-gray-400 ml-1">End Time</label>
                                <input
                                    type="time"
