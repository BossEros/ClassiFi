...
import {
    Search, MoreVertical, Mail, Calendar, CheckCircle, XCircle,
    RefreshCw, Loader2, Trash2, UserPlus,
    ChevronLeft, ChevronRight, Filter, ChevronDown, User as UserIcon
} from 'lucide-react'
...
export function AdminUsersPage() {
    ...
    const getRoleBadgeStyle = (role: string) => {
        switch (role) {
            case 'admin': return 'bg-purple-500/10 text-purple-400 border-purple-500/20 shadow-[0_0_10px_rgba(168,85,247,0.1)]'
            case 'teacher': return 'bg-blue-500/10 text-blue-400 border-blue-500/20 shadow-[0_0_10px_rgba(59,130,246,0.1)]'
            case 'student': return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.1)]'
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-gray-400 mt-1 text-sm">Manage system access, roles, and permissions. <span className="text-gray-500">({total} users)</span></p>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 p-3">
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform ${showFilterDropdown ? 'rotate-180' : ''}`} />
                            </button>
                                    ...
                                    <div className="p-1.5 space-y-0.5">
                                        {(['all', 'student', 'teacher', 'admin'] as const).map((role) => (
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-all border border-transparent ${roleFilter === role
                                                    ? 'bg-blue-500/10 text-blue-400 border-blue-500/10 shadow-[0_0_10px_rgba(59,130,246,0.1)]'
                                                    : 'text-gray-400 hover:bg-white/5 hover:text-white'
                                                    }`}
                                            >
                                                <span className="capitalize font-medium">{role === 'all' ? 'All Roles' : role}</span>
                                                {roleFilter === role && <CheckCircle className="w-3.5 h-3.5 text-blue-400" />}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                {isLoading ? (
                                    ...
                                    [...Array(5)].map((_, i) => (
                                        <tr key={i} className="animate-pulse">
                                            ...
                                            <td className="px-6 py-4"><div className="h-8 w-8 ml-auto bg-white/5 rounded" /></td>
                                        </tr>
                                    ))
                                ) : users.length > 0 ? (
                                    users.map((user) => (
                                        <tr key={user.id} className="group hover:bg-white/[0.02] transition-colors duration-200">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border ${getRoleBadgeStyle(user.role)}`}>
                                                    {user.role.charAt(0).toUpperCase() + user.role.slice(1)}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-2 h-2 rounded-full ${user.isActive ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-gray-500'}`} />
                                                    <span className={`text-[11px] font-medium ${user.isActive ? 'text-gray-300' : 'text-gray-500'}`}>
