...
export function EmailConfirmationPage({ onRedirectToLogin }: EmailConfirmationPageProps) {
  ...
  useEffect(() => {
    ...
    const handleEmailConfirmation = () => {
      ...
      if (!tokenHash) {
        ...
        tokenHash = hashParams.get('access_token') || hashParams.get('token_hash')
      ...
      if (error) {
        // Handle error
        setStatus('error')
        setMessage(errorDescription || 'Failed to confirm email. The link may have expired.')
        return
      }
      ...
      if (tokenHash && (tokenType === 'signup' || tokenType === 'email')) {
        ...
        setStatus('error')
        setMessage('No confirmation data found. Please click the link in your email.')
      }
    }
