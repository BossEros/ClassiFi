...
import {
    Search, MoreVertical, Users, User, Archive,
    RefreshCw, Loader2, Trash2, ChevronLeft, ChevronRight,
    Filter, ChevronDown, CheckCircle, XCircle, Plus, BookOpen
} from 'lucide-react'
...
export function AdminClassesPage() {
    ...
    const [activeDropdown, setActiveDropdown] = useState<{ id: number; x: number; y: number } | null>(null)
    ...
    const fetchClasses = useCallback(async () => {
        try {
            ...
            const response = await adminService.getAllClasses({
                ...
                academicYear: academicYearFilter === 'all' ? undefined : academicYearFilter,
            })
    ...
    }, [page, limit, debouncedSearch, statusFilter, yearLevelFilter, semesterFilter, academicYearFilter])
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-gray-400 mt-1 text-sm">Manage all classes, reassign teachers, and archive classes. <span className="text-gray-500">({total} classes)</span></p>
                    </div>
                ...
                <div className="relative z-50 p-1 rounded-2xl bg-slate-900/50 backdrop-blur-md border border-white/5">
                    <div className="flex flex-col md:flex-row gap-4 p-3">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <span className="capitalize">
                                        {yearLevelFilter === 'all'
                                            ? 'All Years'
                                            : yearLevelFilter === 1 ? '1st Year'
                                                : yearLevelFilter === 2 ? '2nd Year'
                                                    : yearLevelFilter === 3 ? '3rd Year'
                                                        : '4th Year'}
                                    </span>
                                </div>
                                    ...
                                    <div className="p-1.5 space-y-0.5">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-all border border-transparent ${yearLevelFilter === option.value
                                                    ? 'bg-purple-500/10 text-purple-400 border-purple-500/10'
                                                    : 'text-gray-400 hover:bg-white/5 hover:text-white'
                                                    }`}
                                            >
                                                <span className="capitalize font-medium">{option.label}</span>
                                                {yearLevelFilter === option.value && <CheckCircle className="w-3.5 h-3.5 text-purple-400" />}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <span className="capitalize">
                                        {semesterFilter === 'all'
                                            ? 'All Semesters'
                                            : semesterFilter === 1 ? '1st Semester' : '2nd Semester'}
                                    </span>
                                </div>
                                <ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform ${showSemesterDropdown ? 'rotate-180' : ''}`} />
                            </button>
                                    ...
                                    <div className="p-1.5 space-y-0.5">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-all border border-transparent ${semesterFilter === option.value
                                                    ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/10'
                                                    : 'text-gray-400 hover:bg-white/5 hover:text-white'
                                                    }`}
                                            >
                                                <span className="capitalize font-medium">{option.label}</span>
                                                {semesterFilter === option.value && <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <span className="capitalize">{academicYearFilter === 'all' ? 'All A.Y.' : `${academicYearFilter}`}</span>
                                </div>
                                <ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform ${showAcademicYearDropdown ? 'rotate-180' : ''}`} />
                            </button>
                                    ...
                                    <div className="p-1.5 space-y-0.5">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-all border border-transparent ${academicYearFilter === option.value
                                                    ? 'bg-orange-500/10 text-orange-400 border-orange-500/10'
                                                    : 'text-gray-400 hover:bg-white/5 hover:text-white'
                                                    }`}
                                            >
                                                <span className="capitalize font-medium">{option.label}</span>
                                                {academicYearFilter === option.value && <CheckCircle className="w-3.5 h-3.5 text-orange-400" />}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform ${showStatusDropdown ? 'rotate-180' : ''}`} />
                            </button>
                                    ...
                                    <div className="p-1.5 space-y-0.5">
                                        {(['all', 'active', 'archived'] as const).map((status) => (
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-all border border-transparent ${statusFilter === status
                                                    ? 'bg-blue-500/10 text-blue-400 border-blue-500/10 shadow-[0_0_10px_rgba(59,130,246,0.1)]'
                                                    : 'text-gray-400 hover:bg-white/5 hover:text-white'
                                                    }`}
                                            >
                                                <span className="capitalize font-medium">{status === 'all' ? 'All Status' : status}</span>
                                                {statusFilter === status && <CheckCircle className="w-3.5 h-3.5 text-blue-400" />}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-white/5 bg-white/[0.02]">
                                    <th className="px-6 py-5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-[35%]">Class Details</th>
                                    <th className="px-6 py-5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-[20%]">Teacher</th>
                                    <th className="px-6 py-5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-[20%]">Academic Info</th>
                                    <th className="px-6 py-5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-[10%]">Students</th>
                                    <th className="px-6 py-5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-[10%]">Status</th>
                                    <th className="px-6 py-5 text-xs font-semibold text-gray-400 uppercase tracking-wider text-right w-[5%]">Actions</th>
                                </tr>
                            </thead>
                                ...
                                {isLoading ? (
                                    ...
                                    [...Array(5)].map((_, i) => (
                                        <tr key={i} className="animate-pulse">
                                            ...
                                            <td className="px-6 py-5"><div className="h-4 w-24 bg-white/5 rounded" /></td>
                                            <td className="px-6 py-5"><div className="h-4 w-32 bg-white/5 rounded" /></td>
                                            <td className="px-6 py-5"><div className="h-4 w-12 bg-white/5 rounded" /></td>
                                            <td className="px-6 py-5"><div className="h-6 w-16 bg-white/5 rounded-full" /></td>
                                            <td className="px-6 py-5"><div className="h-8 w-8 ml-auto bg-white/5 rounded" /></td>
                                        </tr>
                                    ))
                                ) : classes.length > 0 ? (
                                    classes.map((cls) => (
                                        <tr key={cls.id} className="group hover:bg-white/[0.02] transition-colors duration-200">
                                            ...
                                            <td className="px-6 py-5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-sm text-gray-300 font-medium">{cls.teacherName}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-5">
                                                <div className="space-y-1.5">
                                                    <div className="flex items-center gap-2 flex-wrap">
                                                        <span className="text-[10px] font-medium text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded-full border border-purple-500/10">
                                                            {cls.yearLevel}{cls.yearLevel === 1 ? 'st' : cls.yearLevel === 2 ? 'nd' : cls.yearLevel === 3 ? 'rd' : 'th'} Year
                                                        </span>
                                                        <span className="text-[10px] font-medium text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/10">
                                                            {cls.semester}{cls.semester === 1 ? 'st' : 'nd'} Sem
                                                        </span>
                                                    </div>
                                            ...
                                            <td className="px-6 py-5">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm text-gray-300 font-medium">{cls.studentCount}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-5">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusBadgeStyle(cls.isActive)}`}>
                ...
                <AdminCreateClassModal
                    ...
                    onSuccess={() => {
                        fetchClasses()
                        showToast(classToEdit ? 'Class updated successfully' : 'Class created successfully', 'success')
                    }}
                    teachers={teachers}
                    classToEdit={classToEdit ? {
                        ...
                        teacherId: typeof classToEdit.teacherId === 'string' ? parseInt(classToEdit.teacherId) : classToEdit.teacherId,
