...
import {
    ArrowLeft, Users, Calendar, Clock, BookOpen,
    Search, Trash2, UserPlus, Mail,
...
export function AdminClassDetailPage() {
    ...
    const [activeTab, setActiveTab] = useState<'students' | 'coursework'>('students')
            ...
            <DashboardLayout>
                <div className="flex items-center justify-center min-h-[60vh]">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-gray-400 font-medium">Loading class details...</p>
                    </div>
                </div>
            </DashboardLayout>
        )
            ...
            <DashboardLayout>
                <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                    ...
                    <div className="text-center">
                        <h2 className="text-xl font-bold text-white mb-2">Error Loading Class</h2>
            ...
            <div className="space-y-6 max-w-7xl mx-auto">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <h1 className="text-3xl font-bold text-white tracking-tight">{classInfo.className}</h1>
                                </div>
                                ...
                                <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-gray-400">
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="text-gray-300">{classInfo.teacherName}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Calendar className="w-4 h-4 text-gray-500" />
                                        <span>A.Y. {classInfo.academicYear} • {classInfo.semester === 1 ? '1st' : '2nd'} Sem</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-900/50 border border-white/5 w-fit">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'students'
                    ...
                    >
                        ...
                        Students
                        <span className="ml-1 px-1.5 py-0.5 rounded-md bg-white/10 text-xs">{students.length}</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('coursework')}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'coursework'
                            ? 'bg-white/10 text-white shadow-sm'
                            : 'text-gray-400 hover:text-white hover:bg-white/5'
                            }`}
                    >
                        ...
                        Coursework
                        <span className="ml-1 px-1.5 py-0.5 rounded-md bg-white/10 text-xs">{assignments.length}</span>
                    </button>
                </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b border-white/5 bg-white/[0.02]">
                                        ...
                                        <th className="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Enrolled On</th>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    {studentToRemove?.id === student.id ? (
                                                        <div className="flex items-center justify-end gap-2 animate-in fade-in duration-200">
                                                            <span className="text-xs text-red-400 mr-2">Confirm removal?</span>
                                                            <button
                                ...
                                assignments.map((assignment) => (
                                    ...
                                    >
                                        <div className="flex items-start justify-between mb-3">
                                            ...
                                            {assignment.deadline && (
                                                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${new Date(assignment.deadline) < new Date()
                                        ...
                                        <h3 className="font-semibold text-white mb-1 line-clamp-1">{assignment.title}</h3>
                                        ...
                                        <div className="flex items-center justify-between text-xs text-gray-400 pt-3 border-t border-white/5">
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                <Clock className="w-3.5 h-3.5" />
                                                <span>{assignment.deadline ? new Date(assignment.deadline).toLocaleDateString() : 'No Deadline'}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))
