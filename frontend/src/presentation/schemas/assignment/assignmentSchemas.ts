...
function validateDeadlineAfterScheduledDate(
  deadlineValue: Date,
  scheduledDateValue: Date,
): string | null {
  if (deadlineValue < scheduledDateValue) {
    return "Deadline must be after scheduled date"
  }

  return null
}

interface AssignmentFormSchemaOptions {
...
function createAssignmentFormSchema({
  ...
  return z
    ...
    .superRefine((formValue, context) => {
      ...
      let parsedDeadlineValue: Date | null = null

      if (hasDeadline) {
        parsedDeadlineValue = new Date(formValue.deadline)

        if (requireFutureDeadline) {
          const deadlineError = validateDeadlineValue(parsedDeadlineValue)
        ...
        } else if (Number.isNaN(parsedDeadlineValue.getTime())) {
          context.addIssue({
            code: "custom",
            path: ["deadline"],
      ...
      let parsedScheduledDateValue: Date | null = null
      ...
      if (formValue.scheduledDate) {
        ...
        if (!hasScheduledTime) {
          ...
          parsedScheduledDateValue = new Date(formValue.scheduledDate)
        }
      }

      if (parsedDeadlineValue && parsedScheduledDateValue) {
        const deadlineOrderingError = validateDeadlineAfterScheduledDate(
          parsedDeadlineValue,
          parsedScheduledDateValue,
        )

        if (deadlineOrderingError) {
          context.addIssue({
            code: "custom",
            path: ["deadline"],
            message: deadlineOrderingError,
          })
        }
      }
...
export const assignmentFormSchema = createAssignmentFormSchema({
  requireFutureDeadline: true,
})

export const assignmentEditFormSchema = createAssignmentFormSchema({
