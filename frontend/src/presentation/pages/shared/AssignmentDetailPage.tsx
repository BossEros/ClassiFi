      ...
      >
        <div
          className={cn(
            ...
            isLight ? "border-slate-200 bg-slate-50" : "border-white/5 bg-[#1e1e1e]",
          )}
        >
          <div className="flex flex-col">
            <h3 className={`flex items-center gap-2 text-lg font-semibold tracking-tight ${isLight ? "text-slate-900" : "text-gray-200"}`}>
              {fileName}
            </h3>
            <p className={`font-mono text-xs ${isLight ? "text-slate-500" : "text-gray-500"}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {isCopied ? (
                <>
                  <Check className={`mr-1.5 h-3.5 w-3.5 ${isLight ? "text-emerald-600" : "text-green-400"}`} />
                  <span className={isLight ? "text-emerald-700" : "text-green-400"}>Copied</span>
                </>
              ) : (
                <>
            ...
            <button
              ...
              className={cn(
                ...
                isLight ? "text-slate-400 hover:bg-slate-100 hover:text-slate-900" : "text-gray-400 hover:bg-white/5 hover:text-white",
              )}
            >
              <X className="h-5 w-5" />
            </button>
...
function TeacherFeedbackCard({
  ...
  const handleSave = async () => {
    ...
    try {
      setIsSaving(true)
      const updatedSubmission = await saveSubmissionFeedback(submissionId, feedback)
    ...
    <Card className={isLight ? "border-slate-200 bg-white shadow-sm" : "border-white/10 bg-white/5 backdrop-blur-sm"}>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className={`flex items-center gap-2 ${isLight ? "text-slate-900" : "text-white"}`}>
          <MessageSquare className={`h-5 w-5 ${isLight ? "text-blue-600" : "text-blue-400"}`} />
          Teacher Feedback
        </CardTitle>
        {feedbackGivenAt && (
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex justify-end">
          <Button
            ...
            className={isLight ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-blue-600 text-white shadow-sm shadow-blue-900/20 hover:bg-blue-700"}
          >
...
function SubmissionFeedbackCard({
  ...
  return (
    <Card className={isLight ? "border-blue-200 bg-blue-50/70 shadow-sm" : "border-blue-500/20 bg-blue-500/5 backdrop-blur-sm"}>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className={`flex items-center gap-2 ${isLight ? "text-blue-700" : "text-blue-400"}`}>
          <MessageSquare className="h-5 w-5" />
          Teacher Feedback
        </CardTitle>
        {feedbackGivenAt && (
          <span className={`text-xs ${isLight ? "text-slate-500" : "text-gray-500"}`}>{formatDateTime(feedbackGivenAt)}</span>
        )}
      </CardHeader>
      <CardContent>
        <div className={cn("rounded-lg border p-4", isLight ? "border-blue-200 bg-white" : "border-blue-500/10 bg-black/20")}>
          <p className={`whitespace-pre-wrap text-sm leading-relaxed ${isLight ? "text-slate-700" : "text-gray-200"}`}>{feedback}</p>
        </div>
      </CardContent>
    </Card>
  )
...
export function AssignmentDetailPage() {
  ...
  const parsedSelectedSubmissionId = selectedSubmissionIdValue ? Number(selectedSubmissionIdValue) : NaN
  const selectedSubmissionId = Number.isFinite(parsedSelectedSubmissionId) && parsedSelectedSubmissionId > 0 ? parsedSelectedSubmissionId : null
  ...
  const activeSubmission =
    isTeacher && selectedSubmissionId !== null
      ? (submissions.find((submission) => submission.id === selectedSubmissionId) ?? latestSubmission)
      : latestSubmission
  ...
  const breadcrumbItems = [
    ...
    ...(tempAssignment.classId > 0
      ? [{ label: tempAssignment.className || "Class Overview", to: `/dashboard/classes/${tempAssignment.classId}` }]
      : []),
    {
  ...
  const updateSubmissionInState = (updatedSubmission: Submission) => {
    setSubmissions((previousSubmissions) =>
      previousSubmissions.map((submission) =>
        submission.id === updatedSubmission.id ? { ...submission, ...updatedSubmission } : submission,
      ),
    )
  }
  ...
  const handleOverrideSubmit = async (overriddenGradeValue: number, overrideFeedbackText: string | null) => {
    if (!activeSubmission) {
      return
    }
  ...
  const handleRemoveOverride = async () => {
    ...
    const calculatedAutoGrade =
      submissionTestResults && submissionTestResults.total > 0
        ? Math.floor((submissionTestResults.passed / submissionTestResults.total) * assignmentTotalScore)
        : 0
    ...
    try {
      ...
      showToast(
        removeOverrideError instanceof Error ? removeOverrideError.message : "Failed to remove score override",
        "error",
      )
    }
  }
  ...
  return (
    <DashboardLayout topBar={topBar}>
      {isLoading ? (
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            ...
            ></div>
            <p className={isLightAssignmentView ? "text-slate-500" : "text-gray-400"}>Loading assignment...</p>
          </div>
        </div>
      ) : error && !assignment ? (
        <div className="flex items-center justify-center py-20">
          <div className="max-w-md text-center">
            ...
            >
              <FileCode className={`h-8 w-8 ${isLightAssignmentView ? "text-rose-500" : "text-red-400"}`} />
            </div>
            <p className={`mb-2 font-medium ${isLightAssignmentView ? "text-slate-900" : "text-gray-300"}`}>
              {error.toLowerCase().includes("unauthorized") ? "Access Denied" : "Error Loading Assignment"}
            </p>
            <p className={`mb-4 text-sm ${isLightAssignmentView ? "text-slate-500" : "text-gray-500"}`}>{error}</p>
            {error.toLowerCase().includes("unauthorized") && (
              <p className={`mb-4 text-xs ${isLightAssignmentView ? "text-slate-400" : "text-gray-600"}`}>
                You do not have permission to view this assignment. Make sure you are enrolled in the class.
              </p>
            )}
            <Button
              onClick={() => navigate("/dashboard")}
              className={isLightAssignmentView ? "mx-auto w-auto bg-teal-600 px-4 hover:bg-teal-700" : "mx-auto w-auto"}
            >
              Go to Dashboard
            </Button>
          </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="space-y-6">
                ...
                <CardContent>
                  {hasSubmitted ? (
                    <div className="space-y-4">
                      ...
                      <div className="border-t border-slate-200 pt-4">
                        <p className="mb-1 text-sm text-slate-500">
                          {isTeacher && selectedSubmissionId !== null ? "Selected Submission:" : "Latest Submission:"}
                        </p>
                        ...
                        <div className="mt-3 flex gap-2">
                          <Button
                            onClick={() => activeSubmission && openSubmissionPreview(activeSubmission.id, submissions)}
                          ...
                          <Button
                            onClick={() => activeSubmission && downloadSubmissionFile(activeSubmission.id)}
                            className="h-8 flex-1 border border-slate-300 bg-white text-xs text-slate-700 hover:bg-slate-100 hover:text-slate-900"
                          >
                      ...
                      <div className="border-t border-slate-200 pt-4">
                        ...
                        {activeSubmission?.grade !== undefined && activeSubmission?.grade !== null ? (
                          <div className="flex items-baseline gap-0.5 tabular-nums">
                            <span className={cn("text-2xl font-semibold tracking-tight", getScoreClasses(activeScoreRatio ?? 0))}>
                              {activeSubmission.grade}
                            </span>
                            <span className="text-md font-medium tracking-tight text-slate-600">/{assignmentTotalScore}</span>
                          </div>
                        ) : (
                          <p className="italic text-slate-500">Not graded yet</p>
                        )}
                      </div>

                      {isTeacher && (
                        <div className="space-y-2 border-t border-slate-200 pt-4">
                          <Button onClick={handleOpenOverrideModal} className="h-9 w-full bg-blue-600 text-xs hover:bg-blue-700">
                            Override Score
                          </Button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-slate-900">Not Submitted</p>
                      </div>
                    </div>
                  )}
                </CardContent>
              ...
              {activeSubmission && !isTeacher && activeSubmission.teacherFeedback && (
      ...
      <CodePreviewModal
        ...
        code={previewContent}
        fileName={previewFileName || latestSubmission?.fileName || "Code Preview"}
