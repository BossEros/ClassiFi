...
import { TestResultRepository, type TestResultWithCase } from '@/repositories/testResult.repository.js';
import { SubmissionRepository } from '@/repositories/submission.repository.js';
import { AssignmentRepository } from '@/repositories/assignment.repository.js';
import { StorageService } from './storage.service.js';
import { CODE_EXECUTOR_TOKEN, type ICodeExecutor, type ExecutionRequest } from './interfaces/codeExecutor.interface.js';
...
export class CodeTestService {
     ...
     */
    async runTestsForSubmission(submissionId: number): Promise<TestExecutionSummary> {
        ...
        let sourceCode = await this.storageService.download('submissions', submission.filePath);
        ...
        const newResults: Omit<NewTestResult, 'id' | 'createdAt'>[] = executionResults.map((result, index) => ({
            submissionId,
     ...
     */
    async getTestResults(submissionId: number): Promise<TestExecutionSummary | null> {
        const resultsWithCases = await this.testResultRepo.getWithCasesBySubmissionId(submissionId);
    ...
    private buildResultDetails(
        testCases: TestCase[],
        executionResults: Array<{ status: string; stdout: string | null; stderr: string | null; compileOutput: string | null; executionTimeMs: number; memoryUsedKb: number }>
    ): TestResultDetail[] {
