...
export class AdminEnrollmentService {
     ...
     */
    async getClassStudents(classId: number): Promise<Array<UserDTO & { enrolledAt: string }>> {
        const classExists = await this.classRepo.getClassById(classId);
        if (!classExists) {
            throw new ClassNotFoundError(classId);
        }

        const enrolledStudents = await this.enrollmentRepo.getEnrolledStudentsWithInfo(classId);
