    ...
    async getPairDetails(reportId: string, pairId: number): Promise<PairDetailsResponse> {
        ...
        if (legacyStored) {
            ...
            return {
                ...
                fragments: fragments.map((f: any, i: number) => toPlagiarismFragmentDTO(f, i)),
    ...
    async getReport(reportId: string): Promise<AnalyzeResponse | null> {
        ...
        return {
            ...
            summary: {
                totalFiles: summary.totalFiles,
                totalPairs: summary.totalPairs,
                suspiciousPairs: pairs.filter((p: any) => p.similarity >= threshold).length,
                averageSimilarity: summary.averageSimilarity,
                maxSimilarity: summary.maxSimilarity,
            },
            pairs: pairs.map((pair: any) => toPlagiarismPairDTO(pair)),
            warnings: summary.warnings,
        };
    }
    ...
    private buildAssignmentAnalysisResponse(
        ...
        return {
            ...
            summary: {
                ...
                suspiciousPairs: pairs.filter((p: any) => p.similarity >= PLAGIARISM_CONFIG.DEFAULT_THRESHOLD).length,
                averageSimilarity: parseFloat((pairs.reduce((sum: number, p: any) => sum + p.similarity, 0) / Math.max(1, pairs.length)).toFixed(4)),
                maxSimilarity: parseFloat((Math.max(...pairs.map((p: any) => p.similarity), 0)).toFixed(4)),
            },
