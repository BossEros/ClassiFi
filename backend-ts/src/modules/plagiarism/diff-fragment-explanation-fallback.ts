import type { DiffFragmentExplanation } from "@/modules/plagiarism/diff-fragment-explanation.schema.js"

export interface DiffCodeSelection {
  startRow: number
  startCol: number
  endRow: number
  endCol: number
...
export interface BuildFallbackDiffFragmentExplanationInput {
  leftContent: string
  rightContent: string
...
/**
 * Builds a deterministic fallback explanation for a highlighted diff fragment.
 *
 * @param input - Full file contents plus the selected ranges on both sides.
 * @returns A neutral explanation that can be shown when AI labels are unavailable.
 */
export function buildFallbackDiffFragmentExplanation(
  input: BuildFallbackDiffFragmentExplanationInput,
  ...
  if (isOutputFocusedSnippet(leftSnippet) && isOutputFocusedSnippet(rightSnippet)) {
    return {
  ...
  if (hasChangedLoopCondition(leftSnippet, rightSnippet)) {
    return {
      category: "loop_logic_changed",
...
function countConditionalTokens(snippet: string): number {
  const conditionalMatches = snippet.match(
    /\bif\b|\belif\b|\belse\b|\bswitch\b|\bcase\b/g,
  )
