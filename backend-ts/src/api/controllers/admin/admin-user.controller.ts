...
export async function adminUserRoutes(app: FastifyInstance): Promise<void> {
    ...
    app.patch<{ Params: UserParams; Body: UpdateUserDetails }>('/users/:id/details', {
        ...
        handler: async (request, reply) => {
            const user = await adminUserService.updateUserDetails(request.params.id, request.body);
            return reply.send({ success: true, user });
        },
    });
    ...
    app.patch<{ Params: UserParams; Body: UpdateUserEmail }>('/users/:id/email', {
        ...
        handler: async (request, reply) => {
            const user = await adminUserService.updateUserEmail(request.params.id, request.body.email);
            return reply.send({ success: true, user });
        },
    });
    ...
    app.patch<{ Params: UserParams; Body: UpdateUserRole }>('/users/:id/role', {
        ...
        handler: async (request, reply) => {
            const user = await adminUserService.updateUserRole(request.params.id, request.body.role);
            return reply.send({ success: true, user });
        },
    });
