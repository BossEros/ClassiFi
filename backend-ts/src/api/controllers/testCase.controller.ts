...
export async function testCaseRoutes(app: FastifyInstance): Promise<void> {
    const testCaseRepo = container.resolve<TestCaseRepository>('TestCaseRepository');
     ...
     */
    app.get<{ Params: { assignmentId: string } }>('/assignments/:assignmentId/test-cases', {
        schema: {
            tags: ['Test Cases'],
            summary: 'Get test cases for an assignment',
            params: { type: 'object', properties: { assignmentId: { type: 'string' } } },
            response: {
     ...
     */
    app.post<{ Params: { assignmentId: string }; Body: CreateTestCaseRequest }>('/assignments/:assignmentId/test-cases', {
        schema: {
            tags: ['Test Cases'],
        ...
        handler: async (request, reply) => {
            ...
            const sortOrder = request.body.sortOrder ?? await testCaseRepo.getNextSortOrder(assignmentId);
     ...
     * Update a test case
     */
    app.put<{ Params: { testCaseId: string }; Body: UpdateTestCaseRequest }>('/test-cases/:testCaseId', {
        schema: {
            tags: ['Test Cases'],
            summary: 'Update a test case',
            params: { type: 'object', properties: { testCaseId: { type: 'string' } } },
     ...
     * Reorder test cases
     */
    app.put<{ Params: { assignmentId: string }; Body: ReorderTestCasesRequest }>('/assignments/:assignmentId/test-cases/reorder', {
        schema: {
            tags: ['Test Cases'],
    ...
    app.post<{ Body: RunTestsPreviewRequest }>('/code/run-tests', {
        ...
        handler: async (request, reply) => {
            ...
            const result = await codeTestService.runTestsPreview(sourceCode, language, assignmentId);
     ...
     */
    app.get<{ Params: { submissionId: string } }>('/submissions/:submissionId/test-results', {
        schema: {
            tags: ['Code Testing'],
            summary: 'Get test results for a submission',
            params: { type: 'object', properties: { submissionId: { type: 'string' } } },
            response: {
                200: toJsonSchema(TestResultsResponseSchema),
            },
        },
     ...
     */
    app.post<{ Params: { submissionId: string } }>('/submissions/:submissionId/run-tests', {
        schema: {
            tags: ['Code Testing'],
