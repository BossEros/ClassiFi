...
describe("TeacherDashboardService", () => {
  ...
  describe("getDashboardData", () => {
    ...
    it("should respect limit parameters", async () => {
      ...
      expect(
        mockClassRepo.getRecentClassesWithStudentCounts,
      ).toHaveBeenCalledWith(1, 5)
      expect(mockAssignmentRepo.getPendingTasksForTeacher).toHaveBeenCalledWith(
        1,
        3,
      )
    })
  ...
  describe("getRecentClasses", () => {
    it("should return recent classes with student and assignment counts", async () => {
      ...
      expect(mockAssignmentRepo.getAssignmentsByClassIds).toHaveBeenCalledWith(
        [1, 2],
        true,
      )
