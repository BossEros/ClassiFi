...
export const similarityReports = pgTable('similarity_reports', {
    ...
    averageSimilarity: numeric('average_similarity', { precision: 5, scale: 2 }),
    highestSimilarity: numeric('highest_similarity', { precision: 5, scale: 2 }),
    generatedAt: timestamp('generated_at', { withTimezone: true }).defaultNow().notNull(),
    reportFilePath: text('report_file_path'),
}, (table) => [
    check('check_average_similarity', sql `${table.averageSimilarity} >= 0 AND ${table.averageSimilarity} <= 100`),
    check('check_highest_similarity', sql `${table.highestSimilarity} >= 0 AND ${table.highestSimilarity} <= 100`),
...
export const similarityReportsRelations = relations(similarityReports, ({ one, many }) => ({
...
//# sourceMappingURL=similarity-report.model.js.map