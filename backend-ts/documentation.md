...
- Match View and Diff View label generation is backend-owned. `MatchFragmentExplanationService` asks the shared provider for one batch of Match labels per opened pair, while `DiffFragmentExplanationService` can produce both fragment-level Diff labels and pair-level Diff View targets for all visible changed lines. Pair-level targets include `targetKind` (`changed`, `added`, or `removed`) and may have a `null` left or right selection for one-sided code. Comment-only Match fragments use `comment_text`, and comment-only Diff targets use `comment_changed`. Each returned label is validated with Zod and falls back to deterministic labels on provider errors, refusal, invalid schema, timeout, missing target output, or low confidence. Valid provider responses and fallback responses after failures are stored in bounded in-memory caches keyed by pair content, language, and target selections so reopening the same pair during the same backend process can avoid another AI call. The cache is not persisted and is cleared when the backend restarts. Anthropic returns plain JSON text for parsing; OpenAI uses structured JSON schema output.
...
- Result-detail responses may include pair-level `diffExplanationTargets` for Diff View hover cards across all visible changed lines. Result-detail fragments may also include fragment-level `diffExplanationTargets` for compatibility and evidence export. Each target includes `targetId`, `targetKind`, nullable left/right source ranges, and an explanation object with `category`, `label`, `reasons`, `confidence`, and `source`; `source` is `ai` for validated provider output and `fallback` for deterministic fallback output. The older fragment-level `diffExplanation` remains available as a compatibility fallback.
