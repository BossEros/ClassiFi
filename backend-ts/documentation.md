...
- `vitest` coverage enforces `100%` statements/branches/functions/lines with per-file thresholds for critical contracts (`auth.service`, `auth.schema`, `class.schema`, `class-code.util`, `submission.schema`, `notification-preference.schema`, `notification-preference.service`, `user.service`).
