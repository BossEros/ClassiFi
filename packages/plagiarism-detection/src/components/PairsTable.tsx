...
export const PairsTable: React.FC<PairsTableProps> = ({
    ...
    const SortIndicator: React.FC<{ column: SortKey }> = ({ column }) => {
        ...
        return <span style={{ marginLeft: '4px' }}>{sortOrder === 'desc' ? '↓' : '↑'}</span>;
    };
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: '#fff' }}>
                    <thead>
                        <tr>
                            ...
                            <th style={{ ...headerStyle, textAlign: 'center' }} onClick={() => handleSort('similarity')}>
                                Similarity <SortIndicator column="similarity" />
                            </th>
                            <th style={{ ...headerStyle, textAlign: 'right' }} onClick={() => handleSort('longest')}>
                                Longest Fragment <SortIndicator column="longest" />
                            </th>
                            <th style={{ ...headerStyle, textAlign: 'right' }} onClick={() => handleSort('overlap')}>
                            ...
                            paginatedPairs.map((pair) => {
                                ...
                                return (
                                    <tr
                                        ...
                                        style={{
                                            cursor: 'pointer',
                                            backgroundColor: isSelected ? 'rgba(26, 188, 156, 0.1)' : 'transparent',
                                            transition: 'background-color 0.15s',
                                        }}
                                        onMouseEnter={(e) => {
                                            if (!isSelected) {
                                                e.currentTarget.style.backgroundColor = 'rgba(60, 115, 168, 0.05)';
                                            }
                                        }}
                                        onMouseLeave={(e) => {
                                            if (!isSelected) {
                                    ...
                                    >
                                        ...
                                        <td style={{ ...cellStyle, textAlign: 'center' }}>
                                            <SimilarityBadge similarity={pair.similarity} size="small" showLabel={false} />
                                        </td>
                                        <td style={{ ...cellStyle, textAlign: 'right' }}>{pair.longest} k-grams</td>
                                        <td style={{ ...cellStyle, textAlign: 'right' }}>{pair.overlap}</td>
                                    </tr>
                                );
                            })
                        )}
            ...
            {totalPages > 1 && (
                <div style={{
                    ...
                    <span style={{ color: '#666' }}>
                        ...
                        {Math.min(currentPage * itemsPerPage, filteredPairs.length)} of {filteredPairs.length} pairs
                    </span>
