...
const pairedOccurrence_js_1 = require("./pairedOccurrence.js");
 ...
 */
class Pair extends identifiable_js_1.Identifiable {
    leftEntry;
    rightEntry;
    ...
    rightIgnored;
    constructor(leftEntry, rightEntry) {
        super();
     ...
     */
    buildFragments(minimumOccurrences = 1) {
        ...
        for (const fingerprint of this.shared) {
            ...
            for (let i = 0; i < left.length; i++) {
                const leftOcc = left[i];
                for (let j = 0; j < right.length; j++) {
                    const rightOcc = right[j];
                    const occ = new pairedOccurrence_js_1.PairedOccurrence(leftOcc.side, rightOcc.side, fingerprint);
        ...
        return Array.from(fragmentStart.values())
            .sort((a, b) => range_js_1.Range.compare(a.leftkgrams, b.leftkgrams));
    }
    /**
     * Add a new paired occurrence, extending existing fragments or creating new ones.
     */
    addPair(fragmentStart, fragmentEnd, newPair) {
        ...
        if (fragment) {
            ...
            // Create new fragment
            fragment = new fragment_js_1.Fragment(newPair);
        ...
        if (nextFragment) {
            ...
            fragmentEnd.set(this.key(nextFragment.leftkgrams.to, nextFragment.rightkgrams.to), fragment);
        }
        else {
            fragmentEnd.set(end, fragment);
        }
     ...
     */
    squash(fragmentStart, fragmentEnd) {
        const sortedByStart = Array.from(fragmentStart.values())
            .sort((a, b) => range_js_1.Range.compare(a.leftkgrams, b.leftkgrams));
        const sortedByEnd = Array.from(sortedByStart);
        sortedByEnd.sort((a, b) => range_js_1.Range.compareEnds(a.leftkgrams, b.leftkgrams));
        let j = 0;
        const seen = new Set();
     ...
     */
    removeFragment(fragmentStart, fragmentEnd, fragment) {
        fragmentStart.delete(this.key(fragment.leftkgrams.from, fragment.rightkgrams.from));
        fragmentEnd.delete(this.key(fragment.leftkgrams.to, fragment.rightkgrams.to));
    }
    /**
